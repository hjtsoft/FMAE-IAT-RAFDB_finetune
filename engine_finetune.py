...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # =======================================================
        # 探针：监控 SpatialSaliencyAttention 模块的梯度健康度（仅第一个 Batch）
        # =======================================================
        if epoch == 0 and data_iter_step == 0:
            attn_module = model.module.spatial_attn if hasattr(model, 'module') else model.spatial_attn
            last_linear_grad = attn_module.attn[-1].weight.grad
            gamma_val = attn_module.gamma.item()
            print(f"\n[梯度探针] Gamma 初始值: {gamma_val:.4f}")
            if last_linear_grad is not None:
                print(f"[梯度探针] attn[-1].weight 梯度绝对值均值: {last_linear_grad.abs().mean().item():.2e}")
            else:
                print(f"[梯度探针] attn[-1].weight 梯度: None（loss.backward 尚未执行或梯度断流）")
        # =======================================================
